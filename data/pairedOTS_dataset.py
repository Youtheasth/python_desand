...
class PairedOTSDataset(BaseDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        B_shortPath = '%s.jpg'%A_name.split('_')[0]#'%s%s.png'%('_'.join(A_name.split('_')[:-1]), self.opt.gt_prefix) # '%s.png'%A_name.split('_')[0]
        B_path = os.path.join(self.dir_B, B_shortPath)
        print('A_path,A_name,B_shortPath,B_path')
        print(A_path,A_name,B_shortPath,B_path)
        ...
        seed = np.random.randint(214748364)
        random.seed(seed)
        ...
        return {'haze': A, 'clear': B, 'A_paths': A_path, 'B_paths': B_path,'paths': A_path}
