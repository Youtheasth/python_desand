...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        dcp_J = util.tensor2im(visuals['dcp_J'], np.float) / 255.  # [0, 1]
        # save result images
        if opt.save_image:
            # dehzImg = (result_J*255).astype(np.uint8) #[0, 255], np.uint8
            # util.save_image(dehzImg, os.path.join(curSaveFolder, '%s_dehz.png'%(name)))
            rec_JImg = (rec_J * 255).astype(np.uint8)  # [0, 255], np.uint8
            util.save_image(rec_JImg, os.path.join(curSaveFolder, '%s_rec_J.png' % (name)))
            #refine_JImg = (refine_J * 255).astype(np.uint8)  # [0, 255], np.uint8
            #util.save_image(refine_JImg, os.path.join(curSaveFolder, '%s_refine_J.png' % (name)))

            # refinedT = util.tensor2im(visuals['refine_T_vis'])
            # util.save_image(refinedT, os.path.join(curSaveFolder, '%s_ref_T.png'%(name)))
            dcp_JImg = (dcp_J * 255).astype(np.uint8)  # [0, 255], np.uint8
            #util.save_image(dcp_JImg, os.path.join(curSaveFolder, '%s_dcp_J.png' % (name)))
