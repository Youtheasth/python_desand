...
class RefinedDCPModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_single', 'D_single', 'rec_I', 'TV_T', 'idt_J', 'vgg']  #
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        if self.isTrain:
            self.visual_names = ['real_I', 'dcp_T_vis', 'refine_T_vis', 'out_T_vis', 'dcp_J', 'rec_I', 'rec_J', 'map_A',
                                 'real_J']  # 'refine_J', , 'ref_real_J'
        else:
            self.visual_names = ['real_I', 'dcp_T_vis', 'refine_T_vis', 'out_T_vis', 'dcp_J', 'rec_I', 'rec_J',
                                 'map_A']  # 'refine_J',
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>.
        if self.isTrain:
            self.model_names = ['Refiner_T', 'D']  # , 'Refiner_J'
        else:  # during test time, only load Gs
            self.model_names = ['Refiner_T']  # , 'Refiner_J'

        # define networks (both Generators and discriminators)
        self.netG_DCP = networks.init_net(networks.DCPDehazeGenerator(),
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(
                itertools.chain(self.netRefiner_T.parameters()), lr=opt.lr,
    ...
    def set_input(self, input):
        ...
        # self.real_I = input['A'].to(self.device)  # [-1, 1]
        self.image_paths = input['paths']

        if self.isTrain:
            self.real_J = input['clear'].to(self.device)  # [-1, 1]
            # self.real_J = input['B'].to(self.device)  # [-1, 1]
    ...
    def forward(self):
        ...
        # output scale [0,1]
        self.refine_T, self.out_T = self.netRefiner_T(torch.cat((self.real_I, self.dcp_T), 1))  # self.out_T作用是什么
        print("torch.cat((self.real_I, self.dcp_T), 1).shape=")
        print(torch.cat((self.real_I, self.dcp_T), 1).shape)
        print((self.real_I).shape)
        print( self.dcp_T.shape)

        # self.refine_J = self.netRefiner_J(torch.cat((self.real_I, self.dcp_J), 1))

        # reconstruct haze image
        # shape = self.refine_J.shape
        shape = self.real_I.shape
        ...
        self.rec_I = util.synthesize_fog(self.dcp_J, refine_T_map,
                                         self.map_A)  # 不用refineJ，用dcp_J代替，可以与有雾图像输入比较作为loss函数，有监督
        self.rec_J = util.reverse_fog(self.real_I, refine_T_map, self.map_A)  # 可以与gt比较作为loss函数，无监督,可作为去雾后的结果
    ...
    def backward_D(self):
        ...
        fake_J = self.fake_I_pool.query(self.rec_J)
        self.loss_D_single = self.backward_D_basic(self.netD, self.real_J, fake_J)

    def backward_G(self):
        lambda_idt = self.opt.lambda_identity#无监督时用0#有监督用self.opt.lambda_identity
        lambda_tv = self.opt.lambda_tv
        lambda_G = self.opt.lambda_G #原始代码
        #lambda_G = 0 #为了测试损失函数的消融实验修改参数
        #lambda_rec_I = self.opt.lambda_rec_I#原始代码
        #lambda_rec_I = 0 #为了测试损失函数的消融实验修改参数
        lambda_rec_I = self.opt.lambda_rec_I*0 #由于为0时效果优于权重为1时，所以降低权重为0.01，较差，0.0001更差
        lambda_vgg = self.opt.lambda_vgg

        # Generator losses for rec_I and refine_J
        self.loss_G_single = self.criterionGAN(self.netD(self.rec_J), True) * lambda_G
        ...
        #self.loss_vgg = self.criterionVGG(self.rec_I, self.real_I) * lambda_vgg if lambda_vgg > 0.0 else 0
        self.loss_vgg = self.criterionVGG(self.rec_J, self.real_J) * lambda_vgg if lambda_vgg > 0.0 else 0

        # TV loss
        self.loss_TV_T = self.criterionTV(self.out_T) * lambda_tv if lambda_tv > 0.0 else 0  # 什么用？还要保留吗？

        # Identity loss, ||refiner_J(real_J) - real_J||
        #self.loss_idt_J = self.criterionIdt(self.rec_I, self.real_I) * lambda_idt \

        self.loss_idt_J = self.criterionIdt(self.rec_J, self.real_J) * lambda_idt \
            if lambda_idt > 0.0 \
            else 0
        ...
        '''#self.loss_G_single + self.loss_rec_I + self.loss_idt_J'''
